  ...
  <v-layout id="financials-table">
    <v-flex class="px-2 my-2">
      ...
      <v-flex v-else class="md6 offset-md3">
        <v-card>
          <v-card-title
            class="red headline justify-center white--text"
          >Detailed financials are encrypted</v-card-title>
          <v-card-text>
            <v-text-field
              type="password"
              :error="wrongPassword"
...
const encryptedFinancials = require("@/assets/financials.aes");
...
export default class FinancialsTable extends Vue {
  ...
  public decryptFinancials() {
    try {
      const f = AES.decrypt(encryptedFinancials, this.passphrase).toString(enc.Utf8);
  ...
  public valueFilter(rowName: string, value: number) {
    ...
    if (r.indexOf("cash") >= 0 || r.indexOf("net") >= 0 || r.indexOf("cogs") >= 0 || r.indexOf("sga") >= 0
      || r.indexOf("gross") >= 0) {
      const v = value.toFixed(1).replace(/\d(?=(\d{3})+\.)/g, "$&,");
      return value < 0 ?
        `($${v.substring(1, v.length - 2)})` : `$${v.substring(0, v.length - 2)}`;
    }
